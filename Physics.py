...
class UniformGravity(object):
    ...
    '''
    def __init__(self,solver,a=Vector.Vector(x=0,y=-9.81,z=0),c=0):
        self.a = a
        self.solver = solver
        self.c = c
        solver.diffEq = self.diffEq
    ...
    def diffEq(self,f,t):
        ...
        for j in range(len(self.masses)):
            ...
            dvzdt=self.a.z - self.c*vz**2
            
            dvdt = np.array([dvxdt,dvydt,dvzdt])
            accArray=np.vstack([accArray,dvdt])
        
        return np.array([accArray,velocity])
    ...
    #        print z
            ...
            self.masses=np.append(self.masses,body.mass)
...
#        print f
        ...
        for i in range(len(bodies)):
            bodies[i].position.x = f[0][i][0]
            bodies[i].position.y = f[0][i][1]
            bodies[i].position.z = f[0][i][2]
            
            bodies[i].velocity.x = f[1][i][0]
            bodies[i].velocity.y = f[1][i][1]
            bodies[i].velocity.z = f[1][i][2]
...
#        print 'posArray = ' + repr(posArray)
        ...
        for j in range(len(self.masses)):
            ...
            den=(np.ones(3*len(masses)).reshape(len(masses),3).T*rc).T
            thing = G*(masses*(diff*den).T).T
            thing = np.sum(thing,axis=0)
            dvdt=np.vstack([dvdt,thing])
